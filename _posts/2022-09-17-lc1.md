...
#### 第一步：复制数组
复制一份arr数组用于去重，保存不重复的元素，用来作为取元素大小序号的参考。

#### 第二步：数组去重
在algorithm.h中的unique函数能对容器进行去重，原理是去除掉原容器中相邻的重复的元素。为了将原容器相同的元素都删除，则先应该用algorithm.h中的sort函数对原容器进行排序，使得相同的元素挨在一起。
...
#### 第三步：离散化数组
得到的不包含重复元素的vector作为我们得到元素大小序号的参考，使用algorithm.h中的upper_bound函数来得到每个原数组的大小序号，具体使用方法如下：
